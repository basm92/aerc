...
```{r download_map, fig.height=4, fig.width=8, fig.align='center', warning=F, message=F}
...
```{r demonstration, fig.height=4, fig.width=4, fig.align='center',cache=TRUE}
...
```{r plt, fig.height=4, fig.width=4, fig.align='center', cache=T}
ggplot() + 
  geom_sf(data = fr) + 
...
```{r plot_rds,  fig.height=4, fig.width=6, fig.align='center', cache=T}
ggplot() + 
...
```{r load_sp}
school_performance <- read_delim('../../data/schools/fr-en-indicateurs-valeur-ajoutee-colleges.csv')
```
...
```{r coords}
coordinates <- read_delim('../../data/schools/fr-en-adresse-et-geolocalisation-etablissements-premier-et-second-degre.csv') |>
...
sf_use_s2(F)
nox <- read_stars("../../data/schools/nox_avg_22.tif")
...
```{r plotnox, fig.height=4, fig.width=8, fig.align='center',}
...
modelsummary(model, gof_map = c("r.squared", "nobs"), stars=T)
```
...
```{r schedule, echo=FALSE, results='hold'}
...
- And I can send you the data in due time (Microsoft Teams files)
