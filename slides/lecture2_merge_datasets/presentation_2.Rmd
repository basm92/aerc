...
```{r setup, include=F}
library(tidyverse); library(pacman); library(sf); library(stars)
...
- Also mind the `largest=TRUE` argument if you obtain double matches. 
...
```{r downloadpop, cache=T, results=FALSE}
library(cbsodataR)
...
- The basic libraries you need for importing and changing raster data are called `raster` and `stars`
...
```{r create_potato, fig.width=5, fig.height=3, fig.align='center'}
...
```{r plotcrop, fig.width=5, fig.height=4, fig.align='center'}
...
- Same code using `stars` package
  - You can plot `stars` objects with `ggplot`
...
# Same Thing in `stars` package

```{r fig.width=8, fig.height=5.5, fig.align='center'}
ndvirs <- ndvi_raster |> st_as_stars() 
ndvirs <- ndvirs |> st_warp(crs='wgs84')
ndivrsf <- ndvirs[netherlands]
ggplot() + geom_stars(data=ndivrsf) + scale_fill_viridis_c()
```
...
netherlands_mp <- geodata::gadm("Netherlands", level=2, path="./") |> st_as_sf()
agg <- stats::aggregate(ndvirs, netherlands_mp, FUN=mean, na.rm=T) |> st_as_sf()
agg |> ggplot() + geom_sf(aes(fill=X.1.km.16.days.NDVI.)) + 
  scale_fill_viridis_c(direction=-1, option='turbo')
```
...
per_grid |> 
  st_as_sf() |> 
  ggplot(aes(fill=POP_DENS_2020)) + geom_sf() + scale_fill_viridis_c()
```
